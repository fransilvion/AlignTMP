 ...
 */
public class ViterbiNeedlemanWunsch extends ViterbiAlignment {

    private String stringRepresentation;

    //////////////////////////////////////////////////////////////////////////////////////
    //Viterbi paths for each sequence
    private char[] pathA, pathB;
    ...
    public ViterbiNeedlemanWunsch(Profile targetA, Profile queryB, Gap gapPenalty,
                               HashMap<String, char[]> pathViterbi) throws Exception {
        ...
        this.pathA = pathViterbi.get(targetAids[0]);
        this.pathB = pathViterbi.get(queryBids[0]);
    ...
    protected void process() {
        ...
        for (int i = 1; i <= queryB[0].length; i++) {
            for (int j = 1; j <= targetA[0].length; j++) {
                ...
                double scoreDiagF = mD[i-1][j-1] + Score.viterbiPairwiseScore(quer, targ, pathA, pathB, i-1, j-1);
                double scoreDiagH = H[i-1][j-1] + Score.viterbiPairwiseScore(quer, targ, pathA, pathB, i-1, j-1);
                double scoreDiagV = V[i-1][j-1] + Score.viterbiPairwiseScore(quer, targ, pathA, pathB, i-1, j-1);
                ...
                if (j < targetA[0].length) {
                    ...
                    scoreHF = mD[i-1][j] + gapPenalty.viterbiPairwiseGapFunction(j, i, true, pathA, pathB);
                    //scoreHH = H[i-1][j] + gExt;
                    scoreHH = H[i-1][j] + gapPenalty.viterbiPairwiseGapFunction(j, i, false, pathA, pathB);
                    //scoreHV = V[i-1][j] + gOP;
                    scoreHV = V[i-1][j] + gapPenalty.viterbiPairwiseGapFunction(j, i, true, pathA, pathB);
                }
                else {
                    scoreHF = mD[i-1][j] + 0;
                    scoreHH = H[i-1][j] + 0;
                ...
                if (i < queryB[0].length) {
                    ...
                    scoreVF = mD[i][j-1] + gapPenalty.viterbiPairwiseGapFunction(j, i, true, pathA, pathB);
                    //scoreVV = V[i][j-1] + gExt;
                    scoreVV = V[i][j-1] + gapPenalty.viterbiPairwiseGapFunction(j, i, false, pathA, pathB);
                    //scoreVH = H[i][j-1] + gOP;
                    scoreVH = H[i][j-1] + gapPenalty.viterbiPairwiseGapFunction(j, i, true, pathA, pathB);
                }
                else {
                    scoreVF = mD[i][j-1] + 0;
                    scoreVV = V[i][j-1] + 0;
